...
class IADsGetWorker : public Napi::AsyncWorker {
 ...
 public:
  ...
  void Execute() override {
    ...
    HRESULT hr = m_iads->Get(CComBSTR((const wchar_t *)m_input.c_str()), &var);
    if (FAILED(hr)) {
...
Napi::Value E_IADs::Get(const Napi::CallbackInfo &info) {
  ...
  IADsGetWorker *w = new IADsGetWorker(env, deferred, this->iads, input);
  w->Queue();
